...
namespace argb
{
    bool LuaServerApplication::RequestHandler::process(const HttpRequest& request, HttpResponse& response)
    {
        if (get_state() == State::NOT_STARTED)
        {
            ...
            response_serializer = std::make_unique<HttpResponse::Serializer>(response);
            response_bridge = std::make_unique<LuaHttpResponseSerializerBridge>(*response_serializer);
            ...
            try
            {
                ...
                request_table.set("__native_object_pointer", static_cast<lua::Pointer>(const_cast<HttpRequest*>(current_request)));
                response_table.set("__native_object_pointer", static_cast<lua::Pointer>(response_bridge.get()));
    ...
    void LuaServerApplication::create_bridge_for_server()
    {
        ...
        server_bridge.set("route", [this](lua::Value method_value, lua::Value path_value, lua::Value endpoint_value) {
    ...
    void LuaServerApplication::create_bridge_for_http_request()
    {
        ...
        http_request_metatable.set("__newindex", [this](lua::Value, lua::Value, lua::Value) { virtual_machine.error("HTTP request is read-only."); });
    ...
    void LuaServerApplication::create_bridge_for_http_response()
    {
        ...
        http_response_metatable.set("__newindex", [this](lua::Value, lua::Value, lua::Value) { virtual_machine.error("HTTP response fields cannot be assigned directly."); });
    ...
    {
        if (path.empty() || path.front() != '/') { virtual_machine.error("Parameter 'path' must be a non-empty string starting with '/'."); }
        if (not endpoint.is<lua::Callable>()) { virtual_machine.error("Parameter 'endpoint' must be a Lua function."); }
        auto method = HttpRequest::Parser::method_from_string(method_string);
        if (method == HttpRequest::Method::UNDEFINED) { virtual_machine.error("Invalid HTTP method: %s.", method_string.c_str()); }
    ...
    void LuaServerApplication::create_bridge_for_sqlite()
    {
        ...
        db_table.set("execute", [this](std::string sql, lua::Value args) { auto bind_args = collect_sql_args(args); database().execute_all(sql, bind_args); });
        db_table.set("query", [this](std::string sql, lua::Value args) -> lua::Value { auto bind_args = collect_sql_args(args); return make_sqlite_row_table(database().query_all(sql, bind_args)); });
    ...
    lua::Value LuaServerApplication::make_sqlite_row_table(Sqlite::Row row)
    {
        ...
        auto gc_hook = std::shared_ptr<void>(bridge_ptr, [this](LuaSqliteRowBridge* key) { database_row_bridges.erase(key); });
        ...
        row_metatable.set("__newindex", [this](lua::Value, lua::Value, lua::Value) { virtual_machine.error("Row fields cannot be assigned directly."); });
